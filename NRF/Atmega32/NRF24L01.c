...
#define TX_ADR_WIDTH   	5  // 5字节宽度的发送/接收地址 (5 byte-wide Transmit / Receive Address)
#define TX_PLOAD_WIDTH 	32  // 数据通道有效数据宽度(Data path width of valid data)
...
uint8_t SPI_RW_Reg(uint8_t reg,uint8_t value)
{
	...
	CSN_0;
	status=SPI_RW(reg);   //sFelect register  and write value to it
	SPI_RW(value);   
	CSN_1;
	return(status); 
}
...
void RX_Mode(void)
{
	CE_0;
  	...
  	SPI_RW_Reg(WRITE_REG_NRF24L01 + RF_CH, 40);                 // 选择射频通道0x40(Select the RF channel 0x40)
...
{
/* IRQ(PB4)   CSN(PD6)   CE(PD7) */
/* MISO(PB3)6  MOSI(PB2)5  SCK(PB1)7 SS(PB0) */
